...
const webSsrDev = async ({pro, isSsr}) => {
  ...
  utils.setPortConfig(pro, clientPort, isSsr)
...
const webSpaDev = async ({pro, isSsr}) => {
  console.log(12312)
  const rootPth = process.env.INIT_CWD
  ...
  let clientPort = '8090'
  if (hasConfig) {
    clientPort = await utils.getPort(proVueConfigPath)
  } else {
    clientPort = await utils.getPort(vueConfigPath)
  }
  console.log(333333)
